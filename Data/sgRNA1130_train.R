...
tmp<-c()
for(i in 1:10){
	test.groups.id[[i]]<-as.character(sgRNA.1130.names[which(sgRNA.1130[1:980,6]==i)])
	train.groups.id[[i]]<-as.character(sgRNA.1130.names[which(sgRNA.1130[,6]!=i)])
...
for(j in 1:10){y.test.pred.RF.total[[j]]<-list();length(y.test.pred.RF.total[[j]])<-50}
...
for(n in 1:10){
	...
	for(m in 1:50){
		...
		names(y.test.pred.RF.total[[m]])<-tmp
