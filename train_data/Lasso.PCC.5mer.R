...
PCC.LASSO.5mer.top20<-c()
PCC.LASSO.5mer.all <- c()
...
#			cat("\r",i)
	...
	PCC.LASSO.5mer.top20[j]<-cor(y.test.total[sgRNA.top20.id],y.test.pred.top20[[j]])
	PCC.LASSO.5mer.all[j] <- cor(y.test.total,y.test.pred.total_mean[[j]])
	}
names(y.test.pred.total_mean) <- c("500bp","200bp","100bp","50bp","25bp")
names(y.test.pred.top20) <- c("500bp","200bp","100bp","50bp","25bp")
names(PCC.LASSO.5mer.top20) <-c ("500bp","200bp","100bp","50bp","25bp")
names(PCC.LASSO.5mer.all) <-c ("500bp","200bp","100bp","50bp","25bp")

save(y.test.total,y.test.pred.total_mean,PCC.LASSO.5mer.top20,PCC.LASSO.5mer.all,file="PCC.LASSO.5mer.Rdata")
#}